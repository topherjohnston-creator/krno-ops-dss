...
def block_snow_risk(block_hours: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "source_fxx": best["fxx"],
        "driver": "No snow signal" if prob <= 0 else f"{prob:.1f}% chance {best['label']}",
    }
...
def main() -> None:
    ...
    snow_payload = {
        ...
        "risk_candidates": [c for h in ok_hours for c in h["risk_evaluation"]["candidates"]],
        "methodology": (
    ...
    for hazard in hazards:
        if hazard.get("id") == "SNOW":
            hazard.update(hazard_payload)
            found = True
            break

    if not found:
        hazards.append(hazard_payload)
