...
def latest_available_qmd_cycle() -> datetime:
    """
    Use the newest QMD cycle with enough files for the wind builder.

    Required:
      - f024 for 24-hour max gust mean and 24-hour max gust percentile curve
      - f001-f048 for hourly mean gust timing if available

    The cycle check requires f024 and f048. Individual hourly failures are handled
...
def parse_idx(idx_text: str) -> list[dict[str, Any]]:
    ...
    for i, line in enumerate(lines):
        ...
        try:
            ...
            # Skips malformed HTML / CSS / error lines, e.g. "center; padding".
            continue
...
def extract_percentile_from_line(line: str) -> float | None:
    ...
    # Common IDX wording:
    # GUST:10 m above ground:0-1 day max fcst:50% level:
...
def find_24hr_max_gust_mean_row(idx_rows: list[dict[str, Any]]) -> dict[str, Any] | None:
    """
    Find the QMD 24-hour maximum 10-meter gust mean/deterministic row.

    NOMADS QMD IDX labels the target DESI-like field as:
      GUST:10 m above ground:0-1 day max fcst:

    It does not necessarily say 'ens mean'. Exclude percentile, probability,
...
def find_24hr_max_gust_percentile_rows(idx_rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    return sorted(rows, key=lambda r: float(r["percentile"]))


def find_hourly_gust_mean_row(idx_rows: list[dict[str, Any]], fxx: int) -> dict[str, Any] | None:
    """
    Find QMD hourly mean gust for timing.

    Preferred labels usually resemble:
      GUST:10 m above ground:1 hour fcst:ens mean:
      GUST:10 m above ground:0-1 hour fcst:
      GUST:10 m above ground:0-1 hour fcst:ens mean:

    This intentionally excludes percentile/probability/stddev rows.
    """
    candidates = []
    preferred = []
...
def interpolate_percentile_at_threshold(curve: list[dict[str, Any]], threshold_mps: float) -> float | None:
    """
    Given a percentile curve of gust values, return the percentile corresponding
    to the threshold. Exceedance probability is then 100 - percentile.
    ...
    for i in range(len(clean) - 1):
        ...
        if g0 <= threshold_mps <= g1:
            ...
            fraction = (threshold_mps - g0) / (g1 - g0)
...
def extract_24hr_max_wind(cycle: datetime) -> dict[str, Any]:
    ...
    for key, threshold in WIND_THRESHOLDS.items():
        prob = exceedance_probability_from_curve(
...
def block_wind_timing(block_hours: list[dict[str, Any]]) -> dict[str, Any]:
    valid = [
        h
    ...
    # Timeline is timing/intensity from hourly QMD mean gust. It is not the
    # same as the 24-hr probability matrix used on the risk card.
    if impact <= 1:
        risk = 1 if gust_mph > 0 else 0
    else:
        risk = impact
...
def main() -> None:
    ...
    # Peak timing from hourly QMD mean gust.
    ok_hourly = [
        h
        for h in hourly
        if h.get("status") == "ok" and h.get("gust_mph") is not None
    ...
    for i in range(16):
        ...
        block_eval = block_wind_timing(block_hours)
